...
**Tabla I 100 muestras de tamaño n=10 de una distribución Gamma con parámetros α=3 y σ=2**
...
colnames(resultados) <- c("μ̂₁ (Media muestral)", "μ̂₂ ((min + max)/2)", "μ̂₃ (Mínimo)", "μ̂₄ (Suma/(n+1))")

# Crear tabla con kable
kable(resultados, caption = "Valores estimados de los tiempos de espera por estimador (n = 10)", align = "c", digits = 2)
```
 ...
 **Figura I - Gráfica de curva de densidad Para cada muestra,  del cálculo de  cada uno de los estimadores**
 
 
- Realiza una interpretación de los gráficos obtenidos.


**Según la Figura I - Densidad de μ̂₁EStimador 1 (Media muestral**
...
**Según la Figura I - Densidad de μ̂₂ Estimador 2((min + max)/2)**
...
**Según la Figura I - Densidad de μ̂₃ Estimador 3 (Mínimo)**
...
**Según la Figura I - Densidad de μ̂₄ Estimado 4 (Suma/(n+1))**
...
**Tabla II Comparación de estimadores con la media poblacional**
...
En general, según la Tabla II revela que μ̂ (media muestral) es el estimador con el menor sesgo absoluto (0.130), seguido por μ̂ (0.663), lo que confirma su superioridad en precisión. μ̂ y μ̂ tienen sesgos absolutos altos (0.968 y 4.001, respectivamente), haciendo que sean menos confiables. Este análisis ilustra la importancia de elegir estimadores insesgados como μ̂ para minimizar errores en la estimación.
...
**Figura II - Gráfico de Violín de los Estimadores**

#### Conclusiones:
tal como muestra la figura II El menor sesgo absoluto es **`r round(abs_bias1,2)`**, correspondiente a **μ̂₁ Estimador1 (Media muestral).**
...
**Figura III - Variabilidad relativa de los estimadores vs n**
...
```
**Figura IV - Variabilidad relativa de los estimadores vs n (Área)**
...
**Tabla III - Estadísticas de varianza y coeficiente de variación por estimador y tamaño muestral**
...
results_table <- data.frame(
  n = sample_sizes,
  var1 = round(var_est1, 2),
  cv1 = round(cv_est1, 2),
  var2 = round(var_est2, 2),
  cv2 = round(cv_est2, 2),
  var3 = round(var_est3, 2),
  cv3 = round(cv_est3, 2),
  var4 = round(var_est4, 2),
  cv4 = round(cv_est4, 2)
)

# Renombrar columnas para mayor claridad
colnames(results_table) <- c("n", "Var μ̂1", "CV μ̂1", "Var μ̂2", "CV μ̂2", "Var μ̂3", "CV μ̂3", "Var μ̂4", "CV μ̂4")

# Crear tabla con kable
kable(results_table, caption = "Estadísticas de varianza y coeficiente de variación por estimador y tamaño muestral", align = "c", digits = 2)
```

#### Conclusión comparativa

Así como se observa en la tabla III se piede concluir lo siguiente:
...
**Figura V Sesgo de los estimadores vs n**


```{r grafico_6, echo=FALSE, fig.width=12, fig.height=10, warning = FALSE}
...
**Figura VI Varianza de los estimadores vs n**


```{r grafico_7, echo=FALSE, fig.width=12, fig.height=10, warning = FALSE}
...
**Figura VII MSE de los estimadores vs n**


#### Evaluación de la consistencia
como se observa en la Figuras V, VI y VII se puede concluir que:

μ̂1 (Media muestral): Consistente. Según la figura VI) Sesgo ≈ 0 para n > 20, varianza → 0, MSE → 0. Es el estimador más eficiente.
